...
st.write(
    "Answer the quiz to see how brain-healthy your diet is, "
    "then explore how diet scores relate to sample cognitive test results."
)
...
def ask(question, options, mapping, key):
    """
    Display a selectbox and update global score, healthy, and unhealthy counts.
    """
...
st.write(f"• Healthy choices: **{healthy}**, Unhealthy: **{unhealthy}**")
st.write(f"• Percent healthy: **{percent_healthy}%**")

fig1, ax1 = plt.subplots()
ax1.pie([healthy, unhealthy], labels=["Healthy","Unhealthy"],
...
@st.cache_data
def load_and_prepare(path="brain_diet_data.csv"):
    df = pd.read_csv(path)

    # Rename to match expected logic
    if "greens" in df.columns:
        df.rename(columns={"greens": "leafy_greens"}, inplace=True)

    # Fill in missing columns with zeros
    required_columns = [
        "leafy_greens", "berries", "fish", "nuts", "olive_oil",
        "red_meat", "sweets", "fast_food", "cognitive_score",
        "microbiome_diversity", "amyloid_beta", "tau_protein"
    ]
...
try:
    ...
    slope, intercept, r_val, p_val, _ = linregress(
        df["diet_score"], df["cognitive_score"]
    )
